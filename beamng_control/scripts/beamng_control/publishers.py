...
class ElectricsPublisher(SensorDataPublisher):
    ...
    def _make_msg(self):
        ...
        # TODO: fix the wheels interfacing to be for the four wheels instead of RR only 
        for k, v in data['wheelThermals'].items():
            # info = bng_msgs.WheelThermalsInfo()
            msg.id = k
            msg.brakeSurfaceTemperature = v['brakeSurfaceTemperature']
            msg.brakeThermalEfficiency = v['brakeThermalEfficiency']
            msg.brakeCoreTemperature = v['brakeCoreTemperature']
            msg.wheel_thermals.append(msg)
        return msg
